...
while you wait. Maybe you can report any environmental instabilities you find to someone so
...
step of your history. If that sequence is not all zeroes, repeat this process, using the sequence
you just generated as the input sequence. Once all of the values in your latest sequence are
...
0   3   6   9  12  15   B
  3   3   3   3   3   A
    0   0   0   0   0
You can then start filling in placeholders from the bottom up. A needs to be the result of increasing
3 (the value to its left) by 0 (the value below it); this means A must be 3:
...
Analyze your OASIS report again, this time extrapolating the previous value for each history. What is
the sum of these extrapolated values?
"""
from typing import List


SEQ_STRS = open('./dec_9/input.txt', 'r').readlines()
...
class Sequence:
    ...
    def _add_subseqs(self):
        def not_all_same(seq):
            return seq != [seq[0]] * len(seq)

        cur_seq = self.nums
        while not_all_same(cur_seq):
            next_seq: List[int] = []
            for i in range(0, len(cur_seq) - 1):
                diff = cur_seq[i+1] - cur_seq[i]
                next_seq.append(diff)
            self.subseqs.append(next_seq)
            cur_seq = next_seq
...
def get_diff_to_next_value(sequence: Sequence) -> int:
    diff = 0
    for subseq in sequence.subseqs:
        diff += subseq[-1]
    return diff

def answer_part_1(sequences: List[Sequence]) -> int:
    acc: int = 0
    for seq in sequences:
        acc += get_diff_to_next_value(seq) + seq.nums[-1]
    return acc

def get_diff_to_previous_value(sequence: Sequence) -> int:
    diff = 0
    for subseq in reversed(sequence.subseqs):
        diff = subseq[0] - diff
    return diff

def answer_part_2(sequences: List[Sequence]) -> int:
    acc: int = 0
    for seq in sequences:
        acc += seq.nums[0] - get_diff_to_previous_value(seq)
    return acc

if __name__ == "__main__":
    sequences: List[Sequence] = []
    for sequence_str in SEQ_STRS:
        sequence_str.replace("\n", "")
        num_str_list = sequence_str.split(" ")
        sequence = Sequence(nums=[int(num_str) for num_str in num_str_list])
        sequences.append(sequence)
    print(f"Dec 9 part 1: {answer_part_1(sequences)}")
    print(f"Dec 9 part 2: {answer_part_2(sequences)}")
