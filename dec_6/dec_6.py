...
That must be where the sand comes from! Best of all, the boat races are starting in just a 
few minutes.
...
TIMES_STR, DISTANCES_STR = open('./dec_6/input.txt', 'r').readlines()  # run script from outer dir
TIMES_STR = TIMES_STR[0:-1]  # remove \n
TIMES = [int(s) for s in TIMES_STR.split(" ") if s.isnumeric()]
DISTANCES = [int(s) for s in DISTANCES_STR.split(" ") if s.isnumeric()]

def get_answer_part_1():
    num_ways_multiplied = 1
    for total_time, distance_record in zip(TIMES, DISTANCES):
        num_ways = 0
        for button_time in range(0, total_time + 1):
            speed = button_time
            distance_traveled = (total_time - button_time) * speed
            if distance_traveled >= distance_record:
                num_ways += 1
        num_ways_multiplied *= num_ways
    return num_ways_multiplied

def get_answer_part_2():
    def get_total_num_str(str_array: List[str]):
        acc = ""
        for s in str_array:
            if s.isnumeric():
                acc += s
    ...
    total_time = int(get_total_num_str(TIMES_STR.split(" ")))
    record = int(get_total_num_str(DISTANCES_STR.split(" ")))
    def beats_record(time):
        distance_traveled = (total_time - time) * time
        return distance_traveled >= record
    
    # commence weird binary search
    def get_lower_bound():
        low, high = 0, total_time - 1
        lower_bound = None
        while low <= high:
            mid = (low + high) // 2

            # Hit
            if beats_record(mid) and not beats_record(mid - 1):
                lower_bound = mid
                break
            
            # Too far right
            if beats_record(mid) and beats_record(mid - 1):
                high = mid - 1

            # Too far left
            if not beats_record(mid) and not beats_record(mid - 1):
                low = mid + 1
        return lower_bound
...
if __name__ == "__main__":
    print(f"Dec 6 part 1: {get_answer_part_1()}")
    print(f"Dec 6 part 2: {get_answer_part_2()}")
